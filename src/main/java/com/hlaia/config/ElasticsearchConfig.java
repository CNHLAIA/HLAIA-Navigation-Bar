 ...
 *
 * app.elasticsearch.init-indices 配置开关：
 *   默认值为 true，正常运行时会自动检查并创建索引。
 *   测试环境可以显式设为 false，避免在没有 ES 服务时因为启动期建索引而导致测试失败。
 */
@Slf4j
@Configuration
@ConditionalOnProperty(name = "app.elasticsearch.init-indices", havingValue = "true", matchIfMissing = true)
