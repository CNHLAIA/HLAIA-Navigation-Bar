...
/**
 * 获取当前用户全部书签的导出数据（文件夹树 + 每个文件夹直属书签详情）
 *
 * 供前端 renderExportHtml 渲染成可视化 HTML 后下载。走标准 Result JSON，
 * request.js 拦截器解包后，res.data 即 ExportDataResponse：
 *   { exportedAt, folders: [{ id, name, icon, sortOrder, children, bookmarks }] }
 *
 * @returns {Promise} - res.data = { exportedAt, folders: [...] }
 */
